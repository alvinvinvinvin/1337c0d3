/**
 * The city of Montpellier has equipped its 
 * streets with defibrillators to help save 
 * victims of cardiac arrests. The data 
 * corresponding to the position of all 
 * defibrillators is available online.
...
class Solution {
    ...
    public static void main(String args[]) {
        ...
        int N = in.nextInt();
        in.nextLine();
        List<Defibrillator> lists = new ArrayList<Defibrillator>();
        for (int i = 0; i < N; i++) {
            String DEFIB = in.nextLine();
            lists.add(parse(DEFIB));
        }
        in.close();
        // Write an action using System.out.println()
        // To debug: System.err.println("Debug messages...");
        double min = getDistance(LON_double, LAT_double, lists.get(0).LON, lists.get(0).LAT);
        if(lists.size() == 1){
        ...
        for(int i = 1; i<lists.size(); i++){
        	double temp = getDistance(LON_double, LAT_double, lists.get(i).LON, lists.get(i).LAT);
        	if(min > temp){
        		mark = i;
        		min = temp;
        	}
        ...
        System.out.println(lists.get(mark).name);
    }
    
    static Defibrillator parse(String DEFIB){
    	String defib = DEFIB.replace(',', '.');
    	List<String> defibs = new ArrayList<String>(Arrays.asList(defib.split(";")));
//    	Iterator<String> iterator = defibs.iterator();
...
//    		}
//    	}
    	return new Defibrillator(defibs.get(0), defibs.get(1), 
    			defibs.get(2),defibs.get(3), Double.parseDouble(defibs.get(4)),
    			Double.parseDouble(defibs.get(5)));
    }
    ...
    static class Defibrillator{
    	...
    	double LON = 0;
    	double LAT = 0;
    	public Defibrillator(String id, String name, String address,String phone, double LON, double LAT){
    		this.id = id;
    		this.name = name;
